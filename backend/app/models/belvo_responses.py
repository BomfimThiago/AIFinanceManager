...
class ConnectionSaveRequest(BaseModel):
    ...
    institution: Dict[str, Any] = Field(
        ..., description="Institution data from Belvo widget"
    )
...
class ConsentRenewalRequest(BaseModel):
    ...
    institution_icon_logo: Optional[str] = Field(
...
class ConsentRenewalResponse(BaseModel):
    ...
    consent_renewal_url: str = Field(
        ..., description="URL to Belvo consent renewal portal"
    )
    access_token: str = Field(..., description="Access token for the portal")
    expires_in: int = Field(
        default=3600, description="Token expiration time in seconds"
...
class ConsentExpiredWebhookData(BaseModel):
    ...
    action: str = Field(
        ..., description="Action to take (always 'renew' for expired consents)"
    )
    ...
    institution_icon_logo: Optional[str] = Field(
        None, description="Institution logo URL"
    )
