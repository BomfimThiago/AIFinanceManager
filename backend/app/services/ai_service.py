...
class AIService:
    ...
    async def process_file_with_ai(self, file_content: bytes, file_type: str) -> Optional[List[Expense]]:
    ...
    async def _process_document_full(self, file_content: bytes, file_type: str) -> Optional[List[Expense]]:
    ...
    async def _process_document_chunked(self, file_content: bytes, file_type: str) -> Optional[List[Expense]]:
        ...
        try:
            ...
            if first_batch and len(first_batch) >= 20:
                logger.info(f"Chunked processing: extracted first {len(first_batch)} expenses (document likely has more)")
                return first_batch
            else:
                logger.info(f"Chunked processing: extracted {len(first_batch) if first_batch else 0} expenses total")
                return first_batch
    ...
    def _parse_ai_response(self, response_text: str) -> Optional[List[Expense]]:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Successfully parsed AI response with {len(expenses_data) if isinstance(expenses_data, list) else 1} expenses")
                ...
                try:
                    ...
                    logger.info(f"Successfully repaired and parsed JSON with {len(expenses_data) if isinstance(expenses_data, list) else 1} expenses")
                except json.JSONDecodeError:
                    logger.error(f"JSON repair failed. Raw AI response: {response_text}")
                    return None
            ...
            for i, expense_data in enumerate(expenses_data):
                ...
                if missing_fields:
                    logger.error(f"Expense {i+1} missing required fields: {missing_fields}")
                    continue
