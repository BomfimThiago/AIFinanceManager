...
from ..models.belvo_institution import (
    BelvoInstitutionCreate,
...
class BelvoService:
    ...
    async def convert_to_expenses(
        self, transactions: List[BelvoTransaction]
    ) -> List[ExpenseCreate]:
        """
        ...
        for transaction in transactions:
            if transaction.status != 'PROCESSED':
                logger.info(
                    f"Skipping transaction with status: {transaction.status} - {transaction.id} - {transaction.description}"
                )
                continue
            ...
            if transaction.type == 'INFLOW':
                ...
                logger.info(
                    f"Skipping transaction with unknown type: {transaction.type} - {transaction.id} - {transaction.description}"
                )
                continue
            ...
            try:
                ...
                if expense_type == ExpenseType.INCOME.value:
                    ...
                    category = self._map_belvo_category(
                ...
                logger.info(
                    f"Creating expense with type='{expense_type}', source='{source_value}'"
                )
                ...
                expense = ExpenseCreate(
                    ...
                    or f"Belvo transaction {transaction.id[:8]}",
                ...
                logger.warning(
                    f"Failed to convert Belvo transaction {transaction.id} to expense: {e}"
                )
                logger.debug(f"Failed transaction data: {transaction.dict()}")
                continue

        logger.info(
            f"Converted {len(expenses)} Belvo transactions to expenses from {len(transactions)} total transactions"
        )
        return expenses

    async def get_transaction_by_id(
        self, transaction_id: str
    ) -> Optional[BelvoTransaction]:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status != 200:
                        logger.error(
                            f"Failed to get transaction {transaction_id}: {response.status}"
                        )
                        return None
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                while True:
                    ...
                    url = (
                        f"{self.base_url}/api/transactions/?page={page}&link={link_id}"
                    )
                    ...
                    async with session.get(
                        ...
                        if response.status != 200:
                            error_text = response_body.decode('utf-8')
                            logger.error(
                                f"Failed to get transactions page {page}: {error_text}"
                            )
                            raise Exception(
                                f"Failed to get transactions: {response.status}"
                            )
                        ...
                        if isinstance(result, dict):
                            ...
                            if page == 1:
                                logger.info(
                                    f"Total transactions available: {total_count}"
                                )
                        else:
                        ...
                        logger.info(
                            f"Page {page}: Got {len(page_transactions)} transactions"
                        )
                        ...
                        for txn_data in page_transactions:
                            ...
                            if self._is_valid_transaction(txn_data):
                                try:
                                    ...
                                    logger.warning(
                                        f"Failed to parse transaction {txn_data.get('id', 'unknown')}: {txn_error}"
                                    )
                                    continue
                        ...
                        if page > 100:  # Reasonable limit
                            logger.warning(
                                f"Reached page limit (100) for link {link_id}"
                            )
                            break

            logger.info(
                f"Completed paginated fetch for link {link_id}: {len(all_transactions)} total transactions"
            )
            return all_transactions

        except Exception as e:
            logger.error(
                f"Error in paginated transaction fetch for link {link_id}: {e}"
            )
            raise
    ...
    def _is_valid_transaction(self, txn_data: Dict[str, Any]) -> bool:
        ...
        for field in required_fields:
            if field not in txn_data:
                logger.warning(
                    f"Transaction missing required field {field}: {txn_data.get('id', 'unknown')}"
                )
                return False
        ...
        try:
            ...
            if amount <= 0:
                logger.warning(
                    f"Transaction has invalid amount {amount}: {txn_data.get('id', 'unknown')}"
                )
                return False
        except (ValueError, TypeError):
            logger.warning(
                f"Transaction has non-numeric amount: {txn_data.get('id', 'unknown')}"
            )
            return False
        ...
        try:
            ...
            logger.warning(
                f"Transaction has invalid date: {txn_data.get('id', 'unknown')}"
            )
            return False
    ...
    async def get_transactions_async(
        self, link_id: str, save_data: bool = True
    ) -> Dict[str, Any]:
        ...
        """
        try:
            data = {"link": link_id, "save_data": save_data}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 202:
                        ...
                        result = json.loads(response_body.decode('utf-8'))
                        logger.info(
                            f"Async transaction request accepted for link {link_id}: {result.get('request_id')}"
                        )
                        return result
                        ...
                        error_text = response_body.decode('utf-8')
                        logger.warning(
                            f"Cooldown period active for link {link_id}: {error_text}"
                        )
                        raise Exception(
                            "Cooldown period active. Please wait before making another request."
                        )

                    else:
                        error_text = response_body.decode('utf-8')
                        logger.error(
                            f"Failed to trigger async transactions for link {link_id}: {error_text}"
                        )
                        raise Exception(
                            f"Belvo API error {response.status}: {error_text}"
                        )
    ...
    async def trigger_historical_update(
        self, link_id: str, resources: Optional[List[str]] = None
    ) -> Dict[str, Any]:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status in [200, 202]:
                        result = json.loads(response_body.decode('utf-8'))
                        logger.info(
                            f"Historical update triggered for link {link_id}, resources: {resources}"
                        )
                        return {
                        ...
                        error_text = response_body.decode('utf-8')
                        logger.error(
                            f"Failed to trigger historical update for link {link_id}: {error_text}"
                        )
                        raise Exception(
                            f"Belvo API error {response.status}: {error_text}"
                        )
    ...
    def convert_institution_to_create_model(
        self, institution_data: Dict[str, Any]
    ) -> Optional[BelvoInstitutionCreate]:
        ...
        try:
            ...
            if not all([belvo_id, name, display_name, code]):
                logger.warning(
                    f"Missing required fields in institution {belvo_id}: {institution_data.keys()}"
                )
                return None
