...
class CurrencyService:
    ...
    async def get_current_rates(self, base_currency: Currency = Currency.EUR) -> Dict[str, float]:
        """
    ...
    async def get_historical_rates(self, date: str, base_currency: Currency = Currency.EUR) -> Dict[str, float]:
        """
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Failed to get historical rates for {date}: {response.status}")
    ...
    async def convert_amount(
        ...
        try:
            ...
            if from_currency.value not in exchange_rates or to_currency.value not in exchange_rates:
    ...
    def _get_fallback_rates(self, base_currency: Currency = Currency.EUR) -> Dict[str, float]:
        """
        ...
        logger.warning(f"Using fallback exchange rates with base {base_currency.value}: {fallback_rates}")
    ...
    def detect_currency_from_text(self, text: str) -> Currency:
        ...
        logger.info(f"No currency detected in text, defaulting to EUR")
        return Currency.EUR
