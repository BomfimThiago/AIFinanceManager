...
class CategoryTranslationService:
    ...
    async def translate_category_content(
        ...
        try:
            ...
            fallback = {
                "name": {lang_code: category_name for lang_code in SUPPORTED_LANGUAGES.keys()}
            }
            if category_description:
                fallback["description"] = {lang_code: category_description for lang_code in SUPPORTED_LANGUAGES.keys()}
            return fallback
