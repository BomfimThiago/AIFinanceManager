...
export const useIntegrations = (): UseIntegrationsReturn => {
  ...
  const syncIntegration = useCallback(async (integrationId: number) => {
    try {
      ...
      if (response.ok) {
        showSuccess(
          'Sync Complete!', 
          'Bank transactions have been synchronized successfully.'
        );
        ...
        showError(
          'Sync Failed', 
          'Failed to synchronize transactions. Please try again.'
        );
      }
    } catch (err) {
  ...
  const performDeleteIntegration = useCallback(async (integrationId: number) => {
    try {
  ...
  const requestDeleteIntegration = useCallback((integrationId: number) => {
    const integration = connectedIntegrations.find(int => int.id === integrationId);
    const bankName = integration?.metadata?.display_name || 
                     integration?.metadata?.institution?.display_name || 
                     integration?.institution_name || 'Unknown Bank';
  ...
  const performGetTransactions = useCallback(async (integrationId: number) => {
    try {
      setConfirmationState(prev => ({ ...prev, isLoading: true }));
  ...
  const requestGetTransactions = useCallback((integrationId: number) => {
    const integration = connectedIntegrations.find(int => int.id === integrationId);
    const bankName = integration?.metadata?.display_name || 
                     integration?.metadata?.institution?.display_name || 
                     integration?.institution_name || 'Unknown Bank';
