...
const ReportError: React.FC<{ error: Error }> = ({ error }) => (
  <div className="bg-white p-6 rounded-xl shadow-sm border text-center">
    ...
    <h3 className="text-lg font-semibold text-gray-900 mb-2">Failed to Load Financial Report</h3>
    ...
    <div className="bg-white p-6 rounded-xl shadow-sm border">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-green-50 rounded-lg">
          <h4 className="font-medium text-green-800 mb-2">Under Budget</h4>
          <div className="space-y-1">
            {budget.categories_under_budget.slice(0, 3).map((category: string) => (
              <div key={category} className="text-sm text-green-700 flex items-center">
        ...
        <div className="p-4 bg-red-50 rounded-lg">
          <h4 className="font-medium text-red-800 mb-2">Over Budget</h4>
          <div className="space-y-1">
            {budget.categories_over_budget.slice(0, 3).map((category: string) => (
              <div key={category} className="text-sm text-red-700 flex items-center">
                <AlertTriangle className="h-3 w-3 mr-1" />
                {category}
              </div>
            ))}
            {budget.categories_over_budget.length === 0 && (
              <div className="text-sm text-gray-500">All within budget!</div>
            )}
          </div>
        </div>
      </div>
        ...
        {recommendations.map((rec, index) => (
          <div key={index} className="border border-gray-200 rounded-lg p-4">
            <div className="flex items-start justify-between mb-2">
              ...
              <span
                className={`px-2 py-1 rounded text-xs font-medium border ${getPriorityColor(rec.priority)}`}
              >
                {rec.priority}
              </span>
            </div>
      ...
      {action_plan && (
        <div className="bg-white p-6 rounded-xl shadow-sm border">
          ...
          <div className="mb-4 p-4 bg-green-50 rounded-lg">
            ...
            <div className="text-sm text-green-700">If you follow these recommendations</div>
          </div>
