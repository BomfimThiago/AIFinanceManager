...
const GlobalFiltersSidebar: React.FC<GlobalFiltersSidebarProps> = ({ isVisible, onToggle }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`bg-white border-r border-gray-200 h-full overflow-y-auto flex-shrink-0 transition-all duration-300 ${
          isVisible ? 'w-80' : 'w-0'
        }`}
      >
        <div
          className={`p-6 ${isVisible ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`}
        >
          {/* Header */}
          ...
          <div className="mb-6">
            ...
            <select
              value={filters.type || ''}
              onChange={e =>
                updateFilter('type', e.target.value as 'income' | 'expense' | undefined)
              }
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
